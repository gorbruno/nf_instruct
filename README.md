...
> [Подробнее](https://www.nextflow.io/docs/latest/install.html)
...
Пайплайны отдельно скачивать `не нужно`. Для запуска пайплайна достаточно указать его репозиторий на github. Например:

```bash
nextflow run nf-core\viralrecon <params>
```

Пайплайны `nf-core` можно посмотреть на их [сайте](https://nf-co.re/pipelines/), либо скачать одноимённый пакет _(необязательно)_:
...
> [Подробнее](https://nf-co.re/docs/usage/getting_started/configuration)
...
`test` использует тестовый набор данных, при этом надо указать любой другой параметр (например, docker).
...
Также стоит упомянуть, что команды пайплайнов nextflow можно перечислить в файле с параметрами. Используется флаг `--params-file:
...
Таким образом, для запуска пайплайна глобально нужна одна команда. В ней уточняеся конфигурация и параметры, которые могут меняться в зависимости от задачи. Часто требуется доступ к базе данных, которую надо скачать локально. Все референсные последовательности тоже скачиваются локально в силу последних новостей. Есть параметры, которые позволяют использовать кастомные данные — например, датасет nextclade можно задать через команду `--nextclade_dataset`.
...
nextflow log q
```
Посмотреть какое-то конкретное:
...
Однако, при такой системе временные файлы надо где-то хранить, а весят они много. Но их можно чистить:

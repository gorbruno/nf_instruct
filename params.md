...
- интерфейс написан на Gooey
- в коде куча комментариев, логика создания указана выше

#### utils.py

- небольшие функции для сокращения кода
- load_excel_table берёт на вход эксель файл, при указании страницы и таблицы выводится конкретная таблица 
> даже если на странице 100500 таблиц, но они именованные, то выберется нужная

#### creation.py

- создание текста для файлов параметров и конфигурации
- в коде куча комментариев, логика указана выше

#### create_config.py

- скрипт, обёрнутый в CLI оболочку
- создаёт образец конфига с указанными menu_id
- по команде --help объясняет сам себя

__пример:__

	create_config.py commands.xlsx -s config -t config -p mag
